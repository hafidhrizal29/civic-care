<x-layouts.app>
    <x-slot name="header">
        <h1 class="text-xl font-semibold text-ink">Edit Tanggapan</h1>
    </x-slot>

    <x-breadcrumb :items="[['label' => 'Tanggapan', 'url' => route('responses.index')], ['label' => $response->complaint->nomor_tiket, 'url' => route('complaints.show', $response->complaint)], ['label' => 'Edit']]" />

    <div class="max-w-3xl">
        <form method="POST" action="{{ route('responses.update', $response) }}">
            @csrf
            @method('PUT')
                ...
                <div class="p-6 space-y-5">
                    <div>
                        ...
                        <select name="complaint_id" id="complaint_id" required
                            ...
                            @foreach ($complaints as $complaint)
                                <option value="{{ $complaint->id }}" {{ old('complaint_id', $response->complaint_id) == $complaint->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea name="isi" id="isi" rows="6" required
                                  ...
                                  placeholder="Tuliskan tanggapan Anda...">{{ old('isi', $response->isi) }}</textarea>
                        @error('isi')
                            <p class="mt-1.5 text-sm text-error">{{ $message }}</p>
                        @enderror
                    </div>
