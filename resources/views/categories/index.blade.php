                            ...
                            <tr class="hover:bg-surface-1 transition-colors">
                                ...
                                <td class="px-6 py-3">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <form method="POST" action="{{ route('categories.destroy', $category) }}" x-data="{ confirm: false }" @submit.prevent="if (confirm) { $el.submit(); } else { confirm = true; setTimeout(() => confirm = false, 3000); }">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-1.5 text-ink-subdued hover:text-error hover:bg-error/10 rounded-lg transition-colors" title="Hapus">
                                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
